...
- The same Release run attempted a 188 lb bodyweight check-in. Production rejected `bodyweight_records` with PostgreSQL `42501 permission denied`, leaving the authoritative profile at 187 lb and the website history unavailable. This isolated the missing table grant and led to the local atomic migration/client hardening described above; production acceptance remains pending deployment and a repeat check-in.
...
- Deploy the reviewed `202608090001_atomic_bodyweight_checkins.sql` migration only after explicit authorization, then repeat the fresh-account 188 lb check-in and confirm both current bodyweight and one website history entry update together.

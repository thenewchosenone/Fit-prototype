...
- Production migration history confirms `202608080001` is applied and the `remove-lift-submission` function is live. Signed-in owner-guard testing correctly rejected fixture rows owned by other users; complete the positive-path acceptance with a deliberately created disposable owner submission and verify protected-record cleanup separately.

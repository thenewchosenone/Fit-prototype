...
begin
  ...
  new.leaderboard_eligible_at := case
    ...
    then case when new.evidence_status = 'video_backed'
      then statement_timestamp()
      else coalesce(new.leaderboard_eligible_at, statement_timestamp()) end
    else null end;
...
set leaderboard_eligible_at = statement_timestamp(), updated_at = statement_timestamp()
where evidence_status = 'video_backed'
  and video_asset_id is not null
  and visibility = 'Public'
  and reps = 1
  and is_actual_one_rep_max
  and competitive_movement is not null

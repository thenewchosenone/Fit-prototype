...
Lift Rivals supports Supabase email/password authentication with PKCE. The local development app falls back to an explicitly labeled seeded demo identity when Supabase variables are absent. Production builds do not enable that fallback unless `VITE_AUTH_DEMO_MODE=true` is deliberately supplied.
